...
#include "Editor/Views/Types/AssetBrowserView.h"
...
{
    void HierarchyView::OpenParents(unsigned aParentID)
    {
    ...
    void HierarchyView::HierarchyButton(GameObject* aGameObject, float totalIndent)
    {
        unsigned id = aGameObject->GetID();
        bool goIsOpen = gameobjectIdsThatAreOpen.find(id) != gameobjectIdsThatAreOpen.end();

        ImGui::PushFont(Editor::EditorUI::FontExtraSmall);
    ...
    bool HierarchyView::CheckCopomentType(GameObject* aChild, GameObject* aParent)
    {
    ...
    Canvas* HierarchyView::GetParentCanvas(GameObject* BaseObject)
    {
    ...
    void HierarchyView::AssignParentChildren(GameObject* aChild, GameObject* aParent)
    {
    ...
    void HierarchyView::CreatePrefab(unsigned aGameobjectID, std::filesystem::path aPath)
    {
    ...
    void HierarchyView::Draw()
    {
        /*if (ImGui::BeginPopup("GameobjectHierarchyRightClicked"))
        {
            if (ImGui::BeginMenu("Actions"))
            {
                ...
                {
                    if (AssetBrowserView::ActivePath.has_extension())
                        AssetBrowserView::ActivePath.remove_filename();

                    CreatePrefab(SelectedGameobjectID, AssetBrowserView::ActivePath);
                }
        ...
        {
            unsigned currentObject = HierarchyView::CurrentGameObjectID;
            if (currentObject > 0)
            {
                if (ComponentManager::myEntityIdToEntity.find(currentObject) != ComponentManager::myEntityIdToEntity.end())
                {
                    ...
                    HierarchyView::CurrentGameObjectID = 0;
